...
router.get('/:id', async (req, res) => {
    try {
        ...
        if (error.message.includes("Unable to find Band")) {
...
router.post('/:id', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            if (typeof question !== 'string') throw 'website must be a string';
            if (typeof answer1 !== 'string') throw 'website must be a string';
            if (typeof answer2 !== 'string') throw 'website must be a string';
            if (typeof answer3 !== 'string') throw 'website must be a string';
            if (typeof answer4 !== 'string') throw 'website must be a string';
            if (answer1.trim() === '') throw 'website must be a string';
            if (answer2.trim() === '') throw 'website must be a string';
            if (answer3.trim() === '') throw 'website must be a string';
            if (answer4.trim() === '') throw 'website must be a string';
        } catch (error) {
        ...
        if (error.message.includes("Unable to find Album")) {
            res.status(404).json({ error: error.message });
        } else {
            res.status(500).json({ error: error.message });
        }
